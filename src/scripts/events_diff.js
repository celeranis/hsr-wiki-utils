...
async function getEventsForVersion(version) {
...
function compare(eventSet, prevEventSet) {
    const newNames = eventSet[0];
    const oldNames = prevEventSet[0];
    const newText = eventSet[1];
    const oldText = prevEventSet[1];
    const newOptions = eventSet[2];
    const oldOptions = prevEventSet[2];
    const addedEvents = [];
    const changedEventText = new Map();
            ...
            for (let i = 0; i < newt.length; i++) {
                ...
                for (let i2 = 0; i < newt[i].length; i++) {
                    if (newt[i][i2] != oldt[i][i2]) {
                        if (!changedEventText.has(id))
                            changedEventText.set(id, []);
